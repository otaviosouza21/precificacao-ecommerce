...
export default function ReadXlsx({ setDataPlanilha }: InputFileProps) {
  const [sheetName, setSheetName] = useState("");
  const [uploadStatus, setUploadStatus] = useState<"loading" | "success">(
    "success",
  );
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      const sheet = workbook.SheetNames.find((name) => name === "Renda"); // Aba específica que queremos ler

      if (!sheet) {
        setUploadStatus("success");
        alert(
          "A planilha deve conter uma aba chamada 'Renda'. Por favor, verifique o arquivo e tente novamente.",
        );
        return;
      }
